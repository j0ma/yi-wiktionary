...
def levenshtein_distance(s1, s2):
    return pylev3.Levenshtein.classic(s1, s2)
...
def fetch_next_page_url(list_page_url):
    tree = tree_from_url(list_page_url)
    link_css_tag = "#mw-pages > a"
    a_elements = tree.cssselect(link_css_tag)
    url_suffixes = [
        a.attrib["href"]
        for a in a_elements
        if "pagefrom=" in a.attrib["href"] and "ext page" in a.text
    ]
    if url_suffixes:
        return f"https://en.wiktionary.org/{url_suffixes[-1]}"


def fetch_all_page_urls(url):
    while True:
        yield url
        url = fetch_next_page_url(url)
        if url is None:
            break
...
def fetch_transliteration(word_page_url):
    ...
    yi_word = [
        e.text_content()
        for e in tree.cssselect("strong.headword")
        if is_valid_yiddish(e)
    ][0]
    transliteration = [
        e.text_content()
        for e in tree.cssselect("span.headword-tr")
        if is_valid_transliteration(e)
    ][0]
